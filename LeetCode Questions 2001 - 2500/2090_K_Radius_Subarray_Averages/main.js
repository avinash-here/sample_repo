...
Example 1:  Image Link: https://assets.leetcode.com/uploads/2021/11/07/eg1.png
==============================================================================
...
var getAverages = function(nums, k) {
    if(k > (nums.length-1)/2) return new Array(nums.length).fill(-1);
    let res = [], sum = 0;
    ...
    for(let i = k; i < 2*k; i++)  sum += nums[i];    

    for(let i = k; i < nums.length-k; i++) {
        sum += nums[i+k];
        sum -= nums[i-k-1] == undefined ? 0 : nums[i-k-1];
        res.push(Math.floor(sum/(2*k+1)));
    }

    for(let i = 0; i < k; i++) res.push(-1);
