/*  402. Remove K Digits  // Level: Medium
...
var removeKdigits = function(num, k) {
    let stk = [], nums = num.split('').map(Number),  n = nums.length;
    
    for(let i = 0; i < nums.length; i++) {
        let cur = nums[i];
        while(stk.length > 0 && stk[stk.length-1] > cur && k > 0) {
            stk.pop();  k--;
        } 
        if(stk.length > 0 || cur != 0) stk.push(nums[i]);
    }
