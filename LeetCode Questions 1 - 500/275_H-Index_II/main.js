...
var hIndex = function(citations) {
    let res = find(0, citations.length-1, citations);
    return res != -1  ?  res  :  0;
};
...
function find(l, r, arr) {  
    ...
    while(l <= r) {
        ...
        if(arr[mid] == (arr.length-mid)) return arr.length-mid;
        else if(arr[mid] < (arr.length-mid))  l = mid+1;
        else {
            ans = arr.length-mid;   r = mid-1;
        }
    }
    return ans;
}