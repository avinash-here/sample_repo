/*  2000. Reverse Prefix of Word  // Level: Easy
...
var reversePrefix = function(word, ch) {
    let flag = false,  ind = -1;
    for(let i = 0; i < word.length; i++) {
        if(word[i] == ch) {
            flag = true;  ind = i;  break;
        }
    }
    if(!flag) return word;

    let res = [];
    for(let i = ind; i >= 0; i--) res.push(word[i]);
    for(let i = ind+1; i < word.length; i++) res.push(word[i]);
    return res.join('');
};